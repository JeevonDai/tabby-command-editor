...
| `Enter` | **换行 / Send** | 默认插入普通换行；开启“Enter 发送命令”后发送当前行（或选中内容）到终端，自动去除注释，代码块内会跳过并提示 |
...
| `Ctrl + Enter` | **Send 模式下搜索下一个** | 开启“Enter 发送命令”后，Enter 用于发送，Ctrl+Enter 用于搜索下一个 |
| `Ctrl + Shift + Enter` | **Send 模式下搜索上一个** | 开启“Enter 发送命令”后，Shift+Enter 退出搜索并发送，Ctrl+Shift+Enter 用于搜索上一个 |
| `F8` | **Send** | 同编辑器内 |
...
| 模式 | 行为 |
|---|---|
| **Send (Enter / F8)** | `Enter` 仅在开启“Enter 发送命令”后发送；`F8` 始终发送当前行或选中内容；去除注释；**代码块内禁止** |
...
| **Send** | 发送当前行/选中内容 | `F8`；开启“Enter 发送命令”后也可用 `Enter` |
...
| `codeBlockTerminalCommands` | 发送到当前终端的命令字符串；`{file}` = 临时脚本路径 |
